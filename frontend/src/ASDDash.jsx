...
export default function ASDDash({ studentName, onLogout }) {
  const [schedule, setSchedule] = useState(SCHEDULE)
  const [lessonStep, setLessonStep] = useState(0)

  const currentStep = schedule.find(s => s.status === 'current')
      ...
      <div className="asd-body">
        ...
        <div className="asd-today-bar">
          ...
          <div>
            ...
            <div className="asd-today-val">Biology — Photosynthesis</div>
          </div>
          <div style={{ width: '1.5px', height: 36, background: '#d0ead8' }} />
          <div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div className="asd-header" style={{ marginBottom: 16 }}>
            <div className="asd-title">Photosynthesis — What You Need to Know</div>
            <div className="asd-day-status">There are 5 sections. You are on section {lessonStep + 1} of 5.</div>
          </div>
          ...
          <div style={{
            ...
            }}>{LESSON_CONTENT[lessonStep].heading}</div>
            <div style={{ fontSize: '0.95rem', lineHeight: 1.85, color: '#2a2a2a' }}>
              {LESSON_CONTENT[lessonStep].text}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button className="asd-btn"
              disabled={lessonStep === LESSON_CONTENT.length - 1}
              style={{
                background: lessonStep < LESSON_CONTENT.length - 1 ? 'var(--asd-primary)' : '',
                color: lessonStep < LESSON_CONTENT.length - 1 ? '#fff' : '',
                opacity: lessonStep === LESSON_CONTENT.length - 1 ? 0.3 : 1,
              }}
              onClick={() => setLessonStep(s => Math.min(LESSON_CONTENT.length - 1, s + 1))}>
              Next Section →
            </button>
          </div>
        </div>
