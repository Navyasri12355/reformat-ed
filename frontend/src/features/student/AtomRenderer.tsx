...
export function AtomRenderer({
  ...
  return (
    <div className="atom-renderer">
      <div className="format-badge">{FORMAT_LABEL[atom.output_format] ?? atom.output_format}</div>
      {atom.output_format === "adhd_gamified" && <ADHDFormat atom={atom} />}
      ...
      {atom.output_format === "blended" && <BlendedFormat atom={atom} onAudioPlay={onAudioPlay} />}
    </div>
  );
}
